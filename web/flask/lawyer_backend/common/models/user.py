...
class User(db.Model, TimeModel):
    ...
    status = db.Column(db.Integer, default=1, doc="用户状态 是否可用")
    ...
    role = db.Column(db.Integer, default=1, doc="用户角色 0-管理员 1-普通用户 2-律师")
...
class Lawyer(db.Model):
    ...
    is_certified = db.Column(db.Boolean, default=False, doc="是否已认证 0-未认证 1-认证")

    expert_ident = db.Column(db.String, doc="律师职业证号")
    profile = db.Column(db.String, doc="律师个人信息介绍")

    paid_for_once = db.Column(db.Integer, default=0, doc="单次资讯费用")
    paid_for_hour = db.Column(db.Integer, default=0, doc="每小时资讯费用")
    service_count = db.Column(db.Integer, default=0, doc="已经提供服务人数")
...
class Relation(db.Model, TimeModel):
    ...
    id = db.Column("relation_id", db.Integer, primary_key=True, doc="主键")
    user_id = db.Column(db.Integer, db.ForeignKey("user_base.user_id"), doc="普通用户id")
    target_lawyer_id = db.Column(db.Integer, db.ForeignKey("user_base.user_id"), doc="目标律师的用户ID")
